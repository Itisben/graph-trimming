...
WORKER_LIST="1 16"
SAMPLE_PERCENT="10 20 30 40 50 60 70 80 90"
#WORKER_LIST="16"
...
# results
OUR_RESULT="${BENCHDIR}/results/our-results-large-sample-cmd-6-23-2021.csv"
...
test_real_offline() {
    ...
    for p in `echo ${SAMPLE_PERCENT}`
    do
        echo "Running ${alg} (${algnumber}) on ${OUR_TEST_GRAPH}/${name} with ${workers} worker(s)"
        timeout ${TIMEOUT_TIME} ${OFFLINE_SCC} ${OUR_TEST_GRAPH}/${name} ${workers} ${algnumber} -vs ${p} &> ${TMPFILE}
        echo ${TIMEOUT_TIME} ${OFFLINE_SCC} ${OUR_TEST_GRAPH}/${name} ${workers} ${algnumber} -vs ${p}

        base=${name%.bin} # without the .bin#
        python our-parse-output-sample.py "${base}" "${alg}" "${workers}" "${FAILFOLDER}" "${TMPFILE}" "${OUR_RESULT}" 

        timeout ${TIMEOUT_TIME} ${OFFLINE_SCC} ${OUR_TEST_GRAPH}/${name} ${workers} ${algnumber} -es ${p} &> ${TMPFILE}
        echo ${TIMEOUT_TIME} ${OFFLINE_SCC} ${OUR_TEST_GRAPH}/${name} ${workers} ${algnumber} -es ${p}
