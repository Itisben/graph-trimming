...
//------------------------------------------------------------------------------
inline static void trim_once_node(gm_graph& G, int curr_color, int& count, int &r_count, node_t n)
{
    ...
    /*on-the-fly can not get the reverse edges.*/
#if 0 
    degree = 0;
...
int do_global_trim1(gm_graph& G, int &count, int &r_count)
{
    //int count = 0;
    //int r_count = 0;
    //int travel_edge_cnt = 0;
    ...
    #pragma omp parallel
    {
        ...
        for (node_t n = 0; n < G.num_nodes(); n ++) 
        {
            ...
            trim_once_node(G, curr_color, count_prv, r_count_prv, n);
        }
        __sync_fetch_and_add(&count, count_prv);
        __sync_fetch_and_add(&r_count, r_count_prv);
    }
    //gettimeofday(&T2,NULL);
    //printf("%lf (ms)\n", (T2.tv_sec - T1.tv_sec)*1000 + (T2.tv_usec - T1.tv_usec)*0.001);
    printf("delete v count %d, delete reverse v count %d\n", count, r_count);
...
int do_global_trim1_compact(gm_graph& G)
{
    ...
    #pragma omp parallel
    {
        ...
        for (node_t ix = 0; ix < trim_targets.size(); ix ++) 
        {
            ...
            if (G_Color[n] == -2) continue; // already
            int curr_color = G_Color[n];
            trim_once_node(G, curr_color, count_prv,travel_edge_cnt_prv, n);
        }
        __sync_fetch_and_add(&count, count_prv);
        __sync_fetch_and_add(&travel_edge_cnt, travel_edge_cnt_prv);
    }
    //gettimeofday(&T2,NULL);
    //printf("%lf (ms)\n", (T2.tv_sec - T1.tv_sec)*1000 + (T2.tv_usec - T1.tv_usec)*0.001);
...
int do_local_trim1(gm_graph& G, my_work* w)
{
    ...
    if (set != NULL)
    {
        ...
        for(I=set->begin(); I!=set->end(); I++) {
            trim_once_node(G, curr_color, count, travel_edge_cnt, *I);
        }
        for(I=set->begin(); I!=set->end(); I++) {
    ...
    else 
    {
        for (node_t n = 0; n < G.num_nodes(); n ++) 
        {
            if (G_Color[n] == curr_color) {
                trim_once_node(G, curr_color, count,travel_edge_cnt, n);
            }
        }
    }
...
/*for our test, it should be the real trim that can find all of the v that is a single SCC.*/
int repeat_global_trim1(gm_graph& G, int TRIM_STOP)
{
    ...
    return total_count;
#else /*this is our test, we don't use the trim1_compact. 
        * And the total_count include the reverse trim count.*/
    int total_count = 0;
    int total_r_count = 0;
    int repeat = 0;
    int count=0;
    ...
    do {
        repeat++;
        count=0;r_count=0;
        do_global_trim1(G, count, r_count);
        total_count += count;
        total_r_count += r_count;
    } while (count + r_count > 0);
    printf("trim-repeat-time: %d\n", repeat);
    printf("total-delete-v #: %d\n", total_count);
    printf("total-delte-r-v #: %d\n", total_r_count);
    printf("Total Delete Vertex #: %d\n", total_count + total_r_count);
    return total_count + total_r_count;
#endif
}
