...
inline bool is_trim_dead(node_t v){
    return 0 != atomic_read(&color[v]);
}
void propagation(gm_graph &G, node_t v, int thread_id);

bool g_ufscc_trim = false;
...
start_recursion:
    ...
    while ( 1 ) {
        ...
        for (k_idx = G.begin[v_p]; k_idx < G.begin[v_p+1] ; k_idx ++)
        {
            ...
            /*apply our trim to here, if a node id dead, it is no need to explore*/
            if (is_trim_dead(w)){
                goto NEXTFOR;
            }
...
void do_ufscc_all (gm_graph& G)
{
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for schedule(dynamic, 1024*8)
        for (int i=0; i<G_num_nodes; i++) {
            if(atomic_read(&color[i]) == 0){ /*apply our trim to UFSCC*/
                if (G_Color[i] != -2) {
